...
def reason_step(dataset, prompt_template_manager: PromptTemplateManager, query: str, passages: list, thoughts: list, llm_client: CacheOpenAI):
    ...
    try:
        response_message, metadata, _ = llm_client.infer(messages=messages)
        response_content = response_message if isinstance(response_message, str) else response_message[0]["content"]
